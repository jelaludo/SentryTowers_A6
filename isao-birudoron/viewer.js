...
let loadVersion=0,loading=false,loadError=null;
...
function play(name,loop=true){const source=clip(name);if(!source||!mixer)return null;const action=mixer.clipAction(source);action.reset();action.enabled=true;action.clampWhenFinished=!loop;action.setLoop(loop?T.LoopRepeat:T.LoopOnce,loop?Infinity:1);action.play();return action;}
...
async function loadModel(id){
  const version=++loadVersion,nextEntry=entries.get(id);loading=true;loadError=null;window.isaoViewerReady=false;$('hud').innerHTML='<strong>Loading ISAO…</strong>';
  let loaded;
  try{
    if(!nextEntry)throw new Error('Unknown ISAO model');
    loaded=await loader.loadAsync(`../assets/isao-birudoron/${nextEntry.file}`);
  }catch(error){
    if(version!==loadVersion)return;
    loading=false;loadError=error;$('hud').textContent=`Unable to load ISAO: ${error.message}. Select a model to retry.`;return;
  }
  if(version!==loadVersion){disposeModel(loaded.scene);return;}
  mixer?.stopAllAction();if(wrapper){scene.remove(wrapper);disposeModel(wrapper);}
  rotorAction=hoverAction=emotionAction=toolAction=null;concept={};entry=nextEntry;staticDistance=entry.static===true;production=entry.production_lod===true||staticDistance;
  gltf=loaded;mixer=null;wrapper=new T.Group();wrapper.add(gltf.scene);wrapper.position.y=production?.12:.54;scene.add(wrapper);
  ...
  loading=false;setEmotion(emotion);frameModel();if(!$('hover').checked){stop(rotorAction);stop(hoverAction);}window.isaoViewer={gltf,wrapper,mixer,entry,setEmotion};window.isaoViewerReady=true;
}
function disposeModel(root){
  const geometries=new Set(),materials=new Set(),textures=new Set();
  root.traverse(object=>{if(object.geometry)geometries.add(object.geometry);for(const material of [].concat(object.material||[])){materials.add(material);for(const value of Object.values(material))if(value?.isTexture&&value!==faceTexture)textures.add(value);}});
...
function setEmotion(name){
  ...
  if(!loading&&production&&mixer&&!staticDistance){stop(emotionAction);const clipName=`Emotion_${name[0].toUpperCase()+name.slice(1)}`,meta=entry.clips.find(item=>item.name===clipName);emotionAction=play(clipName,meta?.loop??true);}
...
$('fabricate').onclick=()=>{if(loading||!wrapper)return;if(production&&mixer){stop(toolAction);toolAction=play('Tool_Fabricate');}else setEmotion('working');};
$('model').onchange=()=>loadModel($('model').value);
$('hover').onchange=()=>{if(!loading&&production&&mixer){if($('hover').checked){rotorAction=play('Rotor_Cycle');hoverAction=play('Hover_Idle');}else{stop(rotorAction);stop(hoverAction);}}};
...
renderer.setAnimationLoop(()=>{try{
  ...
  if(loading||loadError||!wrapper){controls.update();renderer.render(scene,camera);return;}
