...
for p in viewers+[P/'index.html',P/'reuse.html',P/'devlog/index.html',P/'roadmap/index.html',P/'best-practices/index.html']:
 ...
 assert links==['/SentryTowers_A6/','/SentryTowers_A6/','/SentryTowers_A6/devlog/','/SentryTowers_A6/roadmap/','/SentryTowers_A6/best-practices/'],(p,links)
 if p in viewers:
 ...
 for t,a in doc.tags:
  ref=a.get('src') if t=='script' else a.get('href') if t=='link' or (p.parent.name in ['devlog','roadmap','best-practices'] and t=='a') else None
  if ref:
...
print(f'PASS {len(viewers)} viewers + 5 library pages: common navigation, base-aware URLs, unique IDs, preserved viewer controls, documentation links and verbatim developer notes.')
