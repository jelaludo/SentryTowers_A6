...
const directory=new URL('../../assets/sol82/',import.meta.url),manifest=JSON.parse(fs.readFileSync(new URL('manifest.json',directory)));
...
assert.equal(manifest.schema,'jelaludo.asset-family/v2-candidate');assert.equal(manifest.family,'sol82_platform');assert.equal(manifest.assets.length,3);assert.equal(manifest.plain_glb_source_of_truth,true);assert.deepEqual(manifest.supported_damage_levels,[0]);assert.equal(manifest.gameplay.beam_vfx_external,true);assert.equal(manifest.gameplay.optical_output_mw,120);assert.equal(manifest.gameplay.burn_budget_s,10);let required=null;
for(const entry of manifest.assets){assert.equal(entry.damage_level,0);assert.equal(entry.state,'intact');assert.equal(entry.origin,'centre of beam exit aperture');assert.equal(entry.fire_direction,'-Y');assert.equal(entry.review_status,'contract_candidate_pending_laser_lab_ground_camera_close_orbit_reference_phone');if(required)assert.deepEqual(entry.required_nodes,required);else required=entry.required_nodes;let plain;
  ...
  else{assert.equal(gltf.animations.length,0);assert.equal(result.skins.length,0);assert.equal(result.drawCalls,1);assert.equal(result.materialNames.length,1);assert.equal(result.materialNames[0],'M_SOL82_DISTANCE_VERTEX_PALETTE');}
...
console.log('PASS SOL-82 plain and decoded Meshopt tiers, names, materials, animation pivots, aperture contract, budgets and self-containment.');
