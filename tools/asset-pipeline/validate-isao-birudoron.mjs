...
const requiredFaceNodes=['FACE_NEUTRAL','FACE_HAPPY','FACE_CURIOUS_LEFT','FACE_CURIOUS_CENTER','FACE_CURIOUS_RIGHT','FACE_WORKING','FACE_ALARM','FACE_DETERMINED','FACE_SAD','FACE_SKEPTICAL_A','FACE_SKEPTICAL_B','FACE_LOVE_SMALL','FACE_LOVE_LARGE'];
const requiredClips=['Rotor_Cycle','Hover_Idle','Emotion_Neutral','Emotion_Happy','Emotion_Curious','Emotion_Working','Emotion_Alarm','Emotion_Determined','Emotion_Sad','Emotion_Skeptical','Emotion_Love','Tool_Fabricate'];
...
for(const entry of manifest.assets.filter(item=>item.production_lod)){
  ...
  for(const emotion of requiredClips.filter(name=>name.startsWith('Emotion_'))){const animation=animations.find(item=>item.getName()===emotion),targets=new Set(animation.listChannels().map(channel=>channel.getTargetNode().getName()));for(const face of requiredFaceNodes)assert(targets.has(face),`${emotion} missing ${face}`);if(emotion!=='Emotion_Neutral')assert([...targets].some(name=>name.startsWith('LEG_')),`${emotion} needs limb performance`);}
  for(const [emotion,faces] of Object.entries({Emotion_Curious:['FACE_CURIOUS_LEFT','FACE_CURIOUS_CENTER','FACE_CURIOUS_RIGHT'],Emotion_Skeptical:['FACE_SKEPTICAL_A','FACE_SKEPTICAL_B'],Emotion_Love:['FACE_LOVE_SMALL','FACE_LOVE_LARGE']})){const animation=animations.find(item=>item.getName()===emotion);for(const face of faces){const channel=animation.listChannels().find(item=>item.getTargetNode().getName()===face&&item.getTargetPath()==='scale'),output=channel.getSampler().getOutput();assert(output.getMin([])[0]<.002&&output.getMax([])[0]>.99,`${emotion} must animate ${face} visibility`);}}
  ...
  assert.deepEqual(nodes.find(node=>node.getName()==='FACE_NEUTRAL').getScale(),[1,1,1]);for(const face of requiredFaceNodes.filter(name=>name!=='FACE_NEUTRAL'))assert.deepEqual(nodes.find(node=>node.getName()===face).getScale(),[.001,.001,.001]);
  ...
  assert.deepEqual(entry.plot_m,[3,3],`${entry.id} plot reservation`);if(entry.lod===1){assert(entry.triangles<=25000,`${entry.id} unit triangle budget`);assert(entry.draw_calls<=40,'game draw-call review ceiling');}assert.equal(root.listTextures().length,0);
...
const viewer=fs.readFileSync(new URL('../../isao-birudoron/viewer.js',import.meta.url),'utf8');assert(viewer.includes('faceTexture.flipY=true'));assert(viewer.includes('Tool_Fabricate'));for(const emotion of ['determined','sad','skeptical','love'])assert(viewer.includes(`${emotion}:`),`viewer missing ${emotion}`);
