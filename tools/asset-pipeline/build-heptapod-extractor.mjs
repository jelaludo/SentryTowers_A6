...
import {buildModel,definitions} from './heptapod-extractor-shape.mjs';
const out=new URL('../../assets/heptapod-extractor/',import.meta.url);
...
for(const [family,d] of Object.entries(definitions))for(const lod of [0,1,2]){
 ...
 const e={id:`${family}_d0_lod${lod}`,family,label:d.label,file,lod,damage_level:0,static:lod===2,derived:false,bytes:bytes.length,sha256:sha(bytes),triangles,draw_calls,materials:new Set((()=>{const m=[];root.traverse(o=>{if(o.isMesh)m.push(o.material);});return m;})()).size,textures:0,meshopt_file,meshopt_bytes:packed.length,meshopt_sha256:sha(packed),meshopt_derived:true,plot_m:d.plot,bounds:{min:b.min.toArray(),max:b.max.toArray(),dimensions_m:b.getSize(new T.Vector3()).toArray()},swept_envelope_m:root.userData.swept_envelope_m||null,sockets,clips:[],engine_nodes,required_nodes:[d.root,...engine_nodes,...sockets.map(s=>s.node)],credit:'Models by jelaludo'};assets.push(e);console.log(e.id,triangles,'triangles',draw_calls,'draws',bytes.length,'bytes');
}
const manifest={schema:'jelaludo.asset-family/v2-candidate',family:'heptapod-extractor',category:'mobile_unit_with_optional_static_distance',label:'HEX-06 / Heptapod resource walker',status:'contract_candidate_pending_game_review',plain_glb_source_of_truth:true,coordinate_system:{units:'meters',up:'+Y',forward:'+Z',origin:'ground at plot centre'},supported_damage_levels:[0],missing_damage_states_reason:'D1–D3 unauthored; damage is independent from detail.',budgets:{lod1:{triangles:8000,draw_calls:10,bytes:400000},lod2:{triangles:3000,draw_calls:1,bytes:250000}},tank_clearance:{width_m:8,height_m:6.5,axis:'+Z',mode:'traffic',reference:'assets/hover-tank/mork_hover_tank_low_d0.glb',reference_size_m:[5.81914,3.01051,13.26736],scope:'Normal tank rest pose, straight passage. Mining laser must be inhibited; turret sweeps and moving-walker passage are not certified.'},lod_selection:{initial:2,approach_distance_m:150,hysteresis_m:20,provisional:true,detailed:'Manual recording and close inspection',distance:'Static parked pose; retain articulated LOD1 for all walking, mining and traffic interactions.'},runtime:{adapter:'runtime.js',baked_clips:[],demonstration:{name:'Extraction_Cycle',duration_s:16,loop:true},scope:'Six-legged mineral-ablation walker. Engine-driven IK legs, laser pivots and preview extraction. Traffic mode inhibits the laser. Walk mode is an in-place tripod demonstration, not navigation.'},assets};
manifest.tank_clearance.reference_sha256=sha(await fs.readFile(new URL('../../assets/hover-tank/mork_hover_tank_low_d0.glb',import.meta.url)));
