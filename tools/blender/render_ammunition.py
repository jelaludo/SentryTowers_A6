...
focus='--arrow' in sys.argv
if focus:families=[f for f in families if f['id']=='tank_arrow']
...
if focus:
 cam.location=(3,-7,3.2);cam.rotation_euler=(Vector((0,0,1.3))-cam.location).to_track_quat('-Z','Y').to_euler();cam.data.ortho_scale=5.2
scene.render.engine='CYCLES';scene.cycles.samples=24;scene.render.resolution_x=1500;scene.render.resolution_y=960;scene.render.resolution_percentage=100;scene.render.image_settings.file_format='PNG';scene.render.filepath='/tmp/ammunition-arrow.png' if focus else '/tmp/ammunition-preview.png';bpy.ops.render.render(write_still=True)
