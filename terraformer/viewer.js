...
const worker=new T.Group();worker.position.set(-9,0,12);scene.add(worker);
...
async function load(){const ticket=++serial;window.terraformerReady=false;const entry=catalog.find(a=>a.family===$('asset').value&&a.damage_level===+$('damage').value);$('status').textContent='Loading module…';let item;
try{const gltf=await loader.loadAsync('../assets/terraformer/'+entry.file);item={group:gltf.scene,entry};if(ticket!==serial){dispose(item);return;}item.group.traverse(o=>{if(o.isMesh){o.castShadow=true;o.receiveShadow=true;o.userData.original=o.material;}});if(gltf.animations.length){item.mixer=new T.AnimationMixer(item.group);item.action=item.mixer.clipAction(gltf.animations[0]);item.action.play();item.duration=gltf.animations[0].duration;}$('motionPanel').hidden=!item.action;$('timeline').max=item.duration||8;overlays(item);dispose(current);current=item;scene.add(item.group);appearance();fit();showTime();$('triangles').textContent=entry.triangles.toLocaleString();$('plot').textContent=entry.plot_m.join(' × ')+' m';$('description').textContent=entry.description;$('status').textContent=entry.name+' / '+entry.state;$('download').href='../assets/terraformer/'+entry.file;window.terraformerKit={current,catalog,scene,camera,orbit};window.terraformerReady=true;}catch(e){if(item)dispose(item);$('status').textContent='Load failed: '+e.message;console.error(e);}}
...
try{const response=await fetch('../assets/terraformer/manifest.json');if(!response.ok)throw Error('Manifest unavailable');catalog=(await response.json()).assets;$('asset').replaceChildren(...catalog.filter(a=>a.damage_level===0).map(a=>new Option(a.name,a.family)));await load();}catch(e){$('status').textContent=e.message;console.error(e);}
