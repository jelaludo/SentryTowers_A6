...
For the 3D assets manager. SOL-82 is the orbital laser platform in Stalheart. The name is the owner's homage to SOL-740 and 1982. This brief covers the platform model only: the beam, the scorch, the smoke and the HUD are drawn by the game.


Desired look : a futuristic Satellite with some combat-looking elements to convey that it generates immense energy to power a destructive laser, some moving parts.  I will mostly be used to display as a wireframe look, but let's give it a full look as the other assets, and some lore as to how it works.

-notes about capabilities in game--
...
An orbital platform working for the gunship, in higher orbit. It passes over the base on a timer. While it is overhead, the player holds a continuous beam from orbit and drags it across the planet to burn a line through the swarm.
