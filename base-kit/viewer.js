...
function spec(){return catalog.find(a=>a.id===($('asset').value==='wall'?`wall_standard_d${$('damage').value}`:{corner:'wall_corner_d0',gate:'gate_vehicle_d0',foundation:'foundation_flat_d0'}[$('asset').value]));}
function reset(front=false){const assembly=$('asset').value==='assembly',gate=$('asset').value==='gate',foundation=$('asset').value==='foundation';const distance=assembly?34:gate?18:9;orbit.target.set(0,assembly?1.4:gate?2:foundation?0:1.35,assembly?1.5:0);camera.position.copy(orbit.target).add(new T.Vector3(front?0:.55,front?.04:.36,front?1:.82).multiplyScalar(distance));orbit.update();}
...
async function load(){const ticket=++serial,mode=$('asset').value;selected=mode;$('status').textContent='Loading geometry…';window.baseKitReady=false;gateTarget=null;const previousMode=load.previous;load.previous=mode;
 $('damagePanel').hidden=!['wall','assembly'].includes(mode);$('gatePanel').hidden=!['gate','assembly'].includes(mode);$('download').hidden=mode==='assembly';
 ...
 instances.forEach(dispose);instances=loaded;instances.forEach(i=>scene.add(i.group));ground.position.y=['foundation','assembly'].includes(mode)?-.405:-.005;grid.position.y=ground.position.y+.008;gatePose();if(previousMode!==mode)reset();
 ...
 else{const entry=spec();$('plot').textContent=entry.plot_m.join(' × ')+' m';$('description').textContent=entry.description;$('download').href='../assets/base-kit/'+entry.file;$('status').textContent=entry.name+' / '+entry.state;}
...
try{const response=await fetch('../assets/base-kit/manifest.json');if(!response.ok)throw Error('Manifest unavailable');catalog=(await response.json()).assets;await load();}catch(e){$('status').textContent=e.message;console.error(e);}
