...
Three self-contained 3D add-ons are available in the existing **hover-tank/** viewer under **Transport containers**:
...
Editable source: `../../../source/blender/mork-containers.blend`. The three scene collections are arranged separately in a gallery; animation tracks are muted for the editable default poses.
...
The checks verify valid GLBs, scene inventory, the tank's complete neutral fit, open-door clearance, straight rollout, ground clearance and a stationary parked tank. All three files passed with zero glTF validator warnings. Use the viewer's cutaway to inspect the fit directly.
