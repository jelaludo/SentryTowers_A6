...
The intact detailed source preserves separate objects and named robot pivots. The derived LOD1 complete line batches the scene into one rigid-weighted skinned mesh while preserving the eight useful robot rigs; its tread/roller animation is replaced by an engine- or shader-driven belt treatment. LOD2 is a static one-draw silhouette tier. Reusable standalone modules remain detailed originals and are not silently represented as reviewed runtime LODs.
